...
if ($mode == "adaugare") {
    ...
    if ((empty($_POST["nume"]))&&(empty($_POST["locatie"]))&&(empty($_POST["email"]))&&(empty($_POST["parola"]))) {
        ...
        $sql = "INSERT INTO companii(nume,locatie,email,parola) VALUES ('$nume','$locatie','$email','$parola')";
        echo $sql;
...
//            $results1= mysqli_query($db,$sql1);
            ...
            if (!isset($_POST['agree-term'])) {
                ...
                if ($parola == $repass) {
                    $query = "INSERT INTO users (nume, username,password,type) VALUES ('$username','$email','$parola','3')";
                    mysqli_query($db, $query);
                    print $username;
                    $numar = "INSERT INTO drepturi(IdPage,IdUser) SELECT '3',users.Id FROM users WHERE username='$email'";
                    mysqli_query($db, $numar);
                    $numar1 = "INSERT INTO drepturi(IdPage,IdUser) SELECT '5',users.Id FROM users WHERE username='$email'";
                    mysqli_query($db, $numar1);
                    $numar2 = "INSERT INTO drepturi(IdPage,IdUser) SELECT '12',users.Id FROM users WHERE username='$email'";
                    mysqli_query($db, $numar2);
                    $numar3 = "INSERT INTO drepturi(IdPage,IdUser) SELECT '13',users.Id FROM users WHERE username='$email'";
                    mysqli_query($db, $numar3);
                    $numar4 = "INSERT INTO drepturi(IdPage,IdUser) SELECT '14',users.Id FROM users WHERE username='$email'";
                    mysqli_query($db, $numar4);
                    $numar5 = "INSERT INTO drepturi(IdPage,IdUser) SELECT '17',users.Id FROM users WHERE username='$email'";
