...
//$id = $_POST['id'];
    ...
    $sql = "SELECT id,nume_firma,titlu, nr_joburi_disponibile,carnet_de_conducere,tip_job,nivel_cariera,limbi_straine,locatie,descriere FROM toate_joburiile where id='$id' ";
//$result= mysqli_query($db,$sql);
    ...
    if ($nr == 1) {
        $myrow = $row;
        echo "<form method=\"post\" action=\"companie_editeaza_job.php?id=" . $id . "\">";
        echo "<div class=\"row\">";
        echo "<div class=\"large-8 medium-8 small-12 columns\">";
//        echo "Id: <input type=\"hidden\" name=\"job_id\" value=". $myrow["id"]. ">";
        echo "Nume firma:  <input type=\"text\" name=\"nume_firma\" value=" . $myrow["nume_firma"] . " >";
        echo "Titlu:  <input type=\"text\" name=\"titlu\" value=" . $myrow["titlu"] . " >";
        echo "Joburi disponibile:  <input type=\"text\" name=\"nr_joburi_disponibile\" value=" . $myrow["nr_joburi_disponibile"] . " >";
        echo "Carnet de conducere:  <input type=\"text\" name=\"carnet_de_conducere\" value=" . $myrow["carnet_de_conducere"] . " >";
        echo "Tip job: <input type=\"text\" name=\"tip_job\" value=" . $myrow["tip_job"] . ">";
        echo "Nivel_cariera: <input type=\"text\" name=\"nivel_cariera\" value=" . $myrow["nivel_cariera"] . ">";
        echo "Limbi straine: <input type=\"text\" name=\"limbi_straine\" value=" . $myrow["limbi_straine"] . ">";
    ...
    if (isset($_POST["btnSubmit"])) {
        ...
        $sql_upd = "UPDATE toate_joburiile SET nume_firma='$nume_firma',titlu='$titlu',nr_joburi_disponibile='$nr_joburi_disponibile',carnet_de_conducere='$carnet_de_conducere',tip_job='$tip_job',nivel_cariera='$nivel_cariera',limbi_straine='$limbi_straine', locatie ='$locatie',descriere='$descriere' where id='$id';";
